...
export async function POST(req: NextRequest, res: NextResponse) {
    ...
    try {
        ...
        if (!room) {
            ...
            await Room.findByIdAndUpdate(roomId, { $set: { player1: "" } });
            response = NextResponse.json({ message: "Left the room", success: true }, { status: 200 });
        } else if (room.player2 == token) {
            await Room.findByIdAndUpdate(roomId, { $set: { player2: "" } });
            response = NextResponse.json({ message: "Left the room", success: true }, { status: 200 });
        }  else  {
            response = NextResponse.json({ message: "You are Not in the room", success: false }, { status: 200 })
        }
    } catch (error) {
